  ...
  title("")
{
  ...
  while (validation_marker == 299.0)
  {
	...
	println("zone_name", z->zone_name);
    readbin(z->parent_zone);     
    readbin(z->strand_id);  
    readbin(z->solution_time);
    readbin(z->not_used);
    readbin(z->zone_type);
	println("zone_type", z->zone_type);

    readbin(z->data_packing);
	println("data_packing", z->data_packing);

    readbin(z->var_location);
	z->var_location = 1;
	println("var_location", z->var_location);
	...
	println("facE_neighbout", z->face_neighbors);
	println("tmp", tmp);
    ...
    if (z->zone_type >= 1 and z->zone_type <= 7)
    {
	  ...
	  //std::cout<<"num_elements"<<z->number_elements<<std::endl;
	  //std::cout<<"num_points"<<z->number_points<<std::endl;
	  //std::cout<<"num_points"<<z->number_points<<std::endl;
	  ...
	  println("should be zero", z->icell);
	  println("should be zero", z->jcell);
	  println("should be zero", z->kcell);
    }
	
    // todo create an auxiliary structure
    // add all the structures to a vector
    readbin(validation_marker);
	println("aux_name", validation_marker);
  ...
  // Data section
  //std::cout<<validation_marker<<std::endl;
  //std::cout<<(int)validation_marker<<std::endl;
  ...
  for (size_t index(0); index<zones.size(); index++)
  {
    ...
    for (int i(0); i<number_variables; i++)
    {
      if (not zones[index]->passive_variables[i])
      {
        ...
        {
			println("switch", zones[index]->variable_format[i]);
          case 1:
          {
...
void tecplotread::read_zone_data(vector<T>& values, int zoneindex, int varindex)
{
  ...
  else 
  {
    ...
    // each particular variables
	  println("varlocationindex", zones[zoneindex]->vars_location[varindex]);
